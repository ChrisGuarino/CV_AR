...
def find_and_warp(frame, source, cornerIDs, arucoDict, arucoParams, useCache=False):
	...
	# if we *did not* find our four ArUco markers, initialize an empty IDs list, otherwise flatten the ID list
	ids = np.array([]) if len(corners) != 4 else ids.flatten()
	# initialize our list of reference points
	refPts = [] 
	# loop over the IDs of the ArUco markers in top-left, top-right,bottom-right, and bottom-left order
	for i in cornerIDs:
		...
		if j.size == 0:
			continue
		# otherwise, append the corner (x, y)-coordinates to our list of reference points
